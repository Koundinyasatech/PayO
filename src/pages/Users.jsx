...
// // ✅ NEW: Helper to extract the creation date from MongoDB ObjectId if createdAt is missing
...
//   // Fetch the wallet address from the profile endpoint
//   useEffect(() => {
...
//   const creationDt = getCreationDate(u); // Extract the date
...
//                   // ✅ Uses the new foolproof date extractor
...
// import { useState, useEffect } from 'react';
// import { getAllUsers } from '../apis/adminApi';
// import { useNavigate } from 'react-router-djhgfsasdfom';
...
//   const [tab, setTab] = useState('details');
//   const [fetchedWallet, setFetchedWallet] = useState(u?.walletAddress || null);
//   const [fetchedBalance, setFetchedBalance] = useState(u?.walletBalance ?? null); // ✅ New state for dynamic balance
//   const [walletLoading, setWalletLoading] = useState(false);
...
//       setWalletLoading(false);
//     });
//   }, [u._id]); // Runs automatically when a specific user is opened
...
//                 {/* ✅ Dynamically loaded balance with loading dots */}
//                 <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 32, fontWeight: 700, letterSpacing: '-1px' }}>
//                   {walletLoading ? '...' : (fetchedBalance ?? u.walletBalance ?? 0).toLocaleString()}
//                   <span style={{ fontSize: 18, opacity: 0.65, marginLeft: 8 }}>PYO</span>
//                 </div>
...
//                 {[
//                   ['Wallet Status',  isVerified ? 'Active' : 'Inactive'],
//                   // ✅ Dynamic Balance updated here as well
//                   ['Balance',        walletLoading ? 'Loading...' : `${(fetchedBalance ?? u.walletBalance ?? 0).toLocaleString()} PYO`],
//                   ['KYC Verified',   isVerified ? 'Yes ✓' : 'No'],
//                 ].map(([l,v]) => (
//                   <div className="detail-item" key={l}><label>{l}</label><span>{v}</span></div>
