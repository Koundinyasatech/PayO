...
import { useNavigate } from 'react-router-dom';
import { useState, useRef, useEffect } from 'react';
import { getDashboardStats, getAllSubmissions, getAllUsers } from '../apis/adminApi';
  ...
  return (
    ...
    >
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12 }}>
        <div style={{ fontSize:10.5, fontWeight:700, color:'var(--stat-card-label, #94A3B8)', textTransform:'uppercase', letterSpacing:'0.7px', lineHeight: 1.3 }}>{label}</div>
      ...
      {loading
        ? <><Skeleton h={30} radius={8} style={{ marginBottom:12 }}/><Skeleton w="70%" h={12} radius={6} style={{ marginBottom:10 }}/><Skeleton h={12} w="40%" radius={6}/></>
        : <>
            <div style={{ fontFamily:"'Space Grotesk', sans-serif", fontSize:24, fontWeight:700, color:'var(--stat-card-value, #0D1B3E)', letterSpacing:'-0.5px', lineHeight:1, marginBottom:12 }}>{value}</div>
            <div style={{ marginBottom:10 }}><Sparkline points={sparkData} color={color}/></div>
            <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, fontWeight:600, color: up ? '#10B981' : '#EF4444' }}>
...
export default function Dashboard() {
  ...
  const [widgets, setWidgets] = useState({ totalTransactions: 0, payoInCirculation: 0, referralRewardsDistributed: 0 });
  ...
  useEffect(() => {
    ...
    // Inline dashboard parameters retrieval to guarantee decoupling from centralized api managers
    const fetchWidgetStatsInline = fetch('https://shadily-hazard-widget.ngrok-free.dev/api/admin/stats/widgets', {
      method: 'GET',
      headers: {
    ...
    Promise.all([
      ...
      getAllSubmissions(),
      getAllUsers(),
      fetchWidgetStatsInline
    ])
      .then(([statsRes, submissionsRes, usersRes, widgetStatsRes]) => {
        const d = statsRes.data?.stats || {};
        setStats(d);

        const arr = submissionsRes.data?.kycs || [];
        setKycs(Array.isArray(arr) ? arr : []);

        setTotals({
          total: usersRes.data?.total ?? 0,
          verified: usersRes.data?.verified ?? 0,
          pending: usersRes.data?.pending ?? 0,
        });

        if (widgetStatsRes?.success) {
          setWidgets({
            totalTransactions: widgetStatsRes.totalTransactions || 0,
            payoInCirculation: widgetStatsRes.payoInCirculation || 0,
            referralRewardsDistributed: widgetStatsRes.referralRewardsDistributed || 0
          });
        }
      })
      .catch((err) => {
        console.error("Dashboard engine data load error:", err);
        setError('Failed to sync complete dashboard parameters');
      })
      .finally(() => {
  ...
  // Filter KYC list by date parameter configurations
  const filteredKyc = (kycs || []).filter(r => {
    if (dateFilter === 'all') return true;
    const d = new Date(r.createdAt);
    const now = new Date();
  ...
  const statCards = [
    { 
      ...
      icon: <svg width="20" height="20" fill="none" stroke="#6C63FF" strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>, 
      sparkData: [30, 35, 45, 38, 52, 58, 62, 70, 68, 79, 84, 92] 
    },
    { 
      label: 'Total Active Wallets', 
      value: totals.verified.toLocaleString(), 
      change: '+11.5% this month', 
      up: true, 
      color: '#10B981', 
      iconBg: 'rgba(16,185,129,0.15)', 
      icon: <svg width="20" height="20" fill="none" stroke="#10B981" strokeWidth="2" viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>, 
      sparkData: [20, 28, 25, 39, 42, 50, 48, 62, 60, 72, 76, 85] 
    },
    { 
      label: 'Total Transactions', 
      value: widgets.totalTransactions.toLocaleString(), 
      change: '+22.8% this month', 
      up: true, 
      color: '#3B82F6', 
      iconBg: 'rgba(59,130,246,0.15)', 
      icon: <svg width="20" height="20" fill="none" stroke="#3B82F6" strokeWidth="2" viewBox="0 0 24 24"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>, 
      sparkData: [45, 52, 49, 63, 58, 70, 74, 82, 79, 91, 96, 104] 
    },
    { 
      label: 'Total PAYO in Circulation', 
      value: `${widgets.payoInCirculation.toLocaleString()} PYO`, 
      change: '+6.2% this month', 
      up: true, 
      color: '#F59E0B', 
      iconBg: 'rgba(245,158,11,0.15)', 
      icon: <svg width="20" height="20" fill="none" stroke="#F59E0B" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>, 
      sparkData: [80, 82, 85, 84, 89, 91, 94, 96, 99, 102, 105, 108] 
    },
    { 
      label: 'Total Referral Distributed', 
      value: `${widgets.referralRewardsDistributed.toLocaleString()} PYO`, 
      change: '+15.1% this month', 
      up: true, 
      color: '#FF6584', 
      iconBg: 'rgba(255,101,132,0.15)', 
      icon: <svg width="20" height="20" fill="none" stroke="#FF6584" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>, 
      sparkData: [15, 22, 18, 29, 34, 40, 38, 46, 51, 55, 62, 70] 
    }
  ];
