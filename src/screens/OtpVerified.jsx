...
export default function OtpVerified({ navigation }) {
  // 1. Modified scale anchor to 0 so the image pops outward from a single point
  const scaleAnim = useRef(new Animated.Value(0)).current;
  const opacityAnim = useRef(new Animated.Value(0)).current;

  // 🚨 PREVENT GOING BACK TO PREVIOUS SCREEN
  useEffect(() => {
    // Disable the iOS swipe-to-go-back gesture
    navigation.setOptions({
      gestureEnabled: false,
    });

    // Block the Android hardware back button and standard back actions
    const unsubscribe = navigation.addListener('beforeRemove', (e) => {
      if (e.data.action.type === 'GO_BACK') {
        e.preventDefault(); // Stops the screen from going back
      }
  ...
  // 2. Optimized Spring physics parameters for a quick dynamic overshoot pop effect
  useEffect(() => {
    Animated.parallel([
      Animated.spring(scaleAnim, {
        toValue: 1,
        friction: 4,          // Softer friction = More elastic snap/bounce effect
        tension: 50,          // Higher tension = Faster accent speed while expanding
        useNativeDriver: true,
      }),
      Animated.timing(opacityAnim, {
        toValue: 1,
        duration: 300,        // Slightly shorter alpha overlay time to match pop speed
        useNativeDriver: true,
      }),
    ]).start();
  }, [scaleAnim, opacityAnim]);
