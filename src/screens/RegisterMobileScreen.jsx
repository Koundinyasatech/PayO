...
export default function RegisterMobileScreen({ navigation }) {
  ...
  navigation.navigate('Home');
    ...
    //   // Exclusively calling the registration OTP endpoint
    //   const response = await api.post('/api/auth/send-otp', { mobile });

    //   console.log('OTP RESPONSE:', response.data);

    //   if (response.data?.message === 'OTP sent') {
    //     // Passing mode as 'register' statically if your OTP screen still expects it
    //     navigation.navigate('OTP', { mobile, mode: 'register' });
    //   } else {
    //     setError(response.data?.message || 'Something went wrong');
    //   }
    // } catch (error) {
