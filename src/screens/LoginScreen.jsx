...
export default function LoginScreen({ navigation }) {
  ...
  const handleLoginWithOTP = async () => {
    ...
    try {
      ...
      console.log('LOGIN API RESPONSE:', response?.data); // 🚨 CHECK YOUR METRO TERMINAL FOR THIS LOG

      if (response?.data?.Message === 'Login OTP generated' || response?.data?.status === '200' || response?.status === 200) {
...
//     // Dynamically prefix the calling code with '+' for the API request
//     const computedCountryCode = `${selectedCountry?.country_calling_code || '91'}`;
//     // const computedCountryCode = callingCode.startsWith('+') ? callingCode : `${callingCode}`;
